...
async def test_select_related():
    async with database:
        ...
        await Track.objects.create(
            album=fantasies, title="Sick Muse", position=2
        )
        await Track.objects.create(
            album=fantasies, title="Satellite Mind", position=3
        )

        track = await Track.objects.select_related("album").get(
            title="The Bird"
        )
        assert track.album.name == "Malibu"
...
async def test_fk_filter():
    async with database:
        ...
        await Track.objects.create(
            album=malibu, title="The Waters", position=3
        )
        ...
        tracks = (
            await Track.objects.filter(album=malibu)
